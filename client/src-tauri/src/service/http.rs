...
pub(crate) async fn put(
    ...
    let client = CLIENT.clone();
    let resp = if body.is_none() {
        client.put(url).headers(header_map).send().await?
    } else {
        client
            .put(url)
            .headers(header_map)
...
pub(crate) async fn post(
    ...
    let client = CLIENT.clone();
    let resp = if body.is_none() {
        client.post(url).headers(header_map).send().await?
    } else {
        client
            .post(url)
            .headers(header_map)
...
mod tests {
    ...
    async fn test() {
        ...
        let resp = super::get("127.0.0.1:11130", "/which_node", &query, headers.as_object().unwrap_or_else(|| {
            &empty_map
        })).await;
        println!("{:?}", resp);
    }
