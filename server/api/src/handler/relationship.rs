...
pub(crate) async fn update_relationship(req: &mut salvo::Request, resp: &mut salvo::Response) {
    ...
    let user_id = user_id.unwrap();
    let req: std::result::Result<UpdateRelationshipReq, ParseError> = req.parse_json().await;
    if req.is_err() {
        resp.render(ResponseResult {
            code: 400,
            message: "bad request.",
    ...
    let req = req.unwrap();
    let res1 = UserRelationship::get_user_id_peer_id(user_id as i64, req.peer_id as i64).await;
    if res1.is_err() {
        resp.render(ResponseResult {
            code: 400,
            message: "not friend.",
    ...
    let res2 = UserRelationship::get_user_id_peer_id(req.peer_id as i64, user_id as i64).await;
    if res2.is_err() {
        resp.render(ResponseResult {
            code: 400,
            message: "not friend.",
