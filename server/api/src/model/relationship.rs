...
impl UserRelationship {
    ...
    pub(crate) async fn get_id(id: i64) -> Result<UserRelationship> {
        let user = sqlx::query_as("SELECT id, user_id, peer_id, remark, status, classification, tag_list, info, create_at, update_at, delete_at FROM api.user_relationship WHERE id = $1 AND delete_at != $2")
            .bind(id)
    ...
    pub(crate) async fn get_user_id_peer_id(
        ...
        let user = sqlx::query_as("SELECT id, user_id, peer_id, remark, status, classification, tag_list, info, create_at, update_at, delete_at FROM api.user_relationship WHERE user_id = $1 AND peer_id = $2 AND delete_at = $3")
            .bind(&user_id)
            .bind(&peer_id)
            .bind(&*crate::DELETE_AT)
            .fetch_one(get_sql_pool().await)
    ...
    pub(crate) async fn get_user_id(
        ...
        number: i64,
        offset: i64,
    ) -> Result<Vec<UserRelationship>> {
        let user = sqlx::query_as("SELECT id, user_id, peer_id, remark, status, classification, tag_list, info, create_at, update_at, delete_at FROM api.user_relationship WHERE user_id = $1 AND delete_at = $2 LIMIT $3 OFFSET $4")
            .bind(&user_id)
            .bind(&*crate::DELETE_AT)
            .bind(&number)
            .bind(&offset)
    ...
    pub(crate) async fn update(&self) -> Result<()> {
        sqlx::query("UPDATE api.user_relationship SET user_id = $1, peer_id = $2, remark = $3, status = $4, classification = $5, tag_list = $6, info = $7, update_at = $8 WHERE id = $9")
            .bind(&self.user_id)
            .bind(&self.peer_id)
            .bind(&self.remark)
            .bind(&self.status)
